...
function drawApps($array) {
	...
	foreach($array as $file => $app) {
		...
		if ($process != false) {
			$rand = rand();
			$return .= '<div id="'.$rand.'" oncontextmenu="Fly.actionmenu(event,[[\'<b>Open</b>\',function() {document.getElementById(\''.$rand.'\').onclick();},{icon:\''.$_FLY['RESOURCE']['URL']['ICONS'].'run.svg\'}],[\'Pin to Jump\',function() {},{icon:\''.$_FLY['RESOURCE']['URL']['ICONS'].'pin.svg\'}],[\'\'],[\'Applications Options\',function(){window.top.system.command(\'run:SprocketComputers.Options,page=filesapps,action=installed\');window.top.ui.jump.toggle();},{icon:\''.$_FLY['RESOURCE']['URL']['ICONS'].'options.svg\'}]]);return false;" onclick="window.top.ui.jump.toggle();window.top.system.eval64(\''.base64_encode('system.command(\'run:'.$process["file"].'\')').'\');" class="FlyUiMenuItem FlyUiText FlyUiNoSelect"><img style="width:24px;height:24px;vertical-align:middle;margin-right:6px;" src="'.$process["icon"].'">'.htmlentities($process["name"]).'';
			...
			if (isset($xml->masks)) {
				...
				foreach ($xml->masks->children() as $mask) {
					if (!in_array((string)$mask['hidden'],['true','on','yes'])) {
						...
						if ($process != false) {
							$rand = rand();
							$m .= '<div id="'.$rand.'" oncontextmenu="Fly.actionmenu(event,[[\'<b>Open</b>\',function() {document.getElementById(\''.$rand.'\').onclick();},{icon:\''.$_FLY['RESOURCE']['URL']['ICONS'].'run.svg\'}],[\'Pin to Jump\',function() {},{icon:\''.$_FLY['RESOURCE']['URL']['ICONS'].'pin.svg\'}],[\'\'],[\'Applications Options\',function(){window.top.system.command(\'run:SprocketComputers.Options,page=filesapps,action=installed\');window.top.ui.jump.toggle();},{icon:\''.$_FLY['RESOURCE']['URL']['ICONS'].'options.svg\'}]]);event.stopPropagation();return false;" onclick="window.top.ui.jump.toggle();window.top.system.eval64(\''.base64_encode('system.command(\'run:'.$process["file"].'\')').'\');event.stopPropagation();" class="FlyUiMenuItem FlyUiText FlyUiNoSelect"><img style="width:24px;height:24px;vertical-align:middle;margin-right:6px;" src="'.$process["icon"].'">'.htmlentities($process["name"]).'</div>';
						}
					}
				}
				if ($m !== '') {
